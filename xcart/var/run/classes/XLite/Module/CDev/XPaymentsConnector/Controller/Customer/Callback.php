...
abstract class Callback extends \XLite\Controller\Customer\CallbackAbstract implements \XLite\Base\IDecorator
{
    ...
    protected function doActionCheckCart()
    {
        ...
        \XLite\Logger::getInstance()->logCustom(
            ...
            'Check cart action. RefId: ' . var_export($refId, true) . PHP_EOL
                . 'Transaction: ' . ($transaction ? $transaction->getPublicTxnId() : 'n/a') . PHP_EOL
    ...
    /**
     * Public detect transaction
     *
     * @return void
     */
    public function callDetectTransaction()
    {
        return $this->detectTransaction();
    }
    ...
    protected function doActionCallback()
    {
        ...
        if (
            ...
            && $transaction->isCallCheckoutSuccessNecessary()
        ) {
            ...
            if (
                ...
                && method_exists($transaction->getOrder(), 'getPrivateAttachments')
                && $transaction->getOrder()->isProcessed()
            ) {
                foreach ($transaction->getOrder()->getPrivateAttachments() as $item) {
                    $item->renew();
                }
