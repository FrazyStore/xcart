 ...
 */
abstract class ConverterAbstract extends \XLite\Base\Singleton
{
    /**
     * Sizes
     */
    ...
    public static function getControllerClass($target)
    {
        ...
        // Initialize cache
        if (!isset(static::$cacheControllers)) {
    ...
    public static function getCleanURLSeparator()
    {
        ...
        if (
            empty($result)
            || !preg_match('/' . static::getCleanURLAllowedCharsPattern() . '/S', $result)
        ) {
     ...
     * @return string
     */
    public static function isCapitalizeCleanURLWords()
    {
    ...
    protected static function getStrftime($format, $base = null)
    {
        ...
        $date = isset($base) ? strftime($format, $base) : strftime($format);
