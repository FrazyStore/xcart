...
class Database extends \XLite\Base\Singleton
{
    ...
    public function loadClassMetadata(\Doctrine\ORM\Event\LoadClassMetadataEventArgs $eventArgs)
    {
        ...
        if ($prefix && !$classMetadata->isMappedSuperclass) {
            ...
            $assignedDiscriminatorMaps = \XLite\Core\Database::getCacheDriver()->fetch('assignedDiscriminatorMaps');
            if (!is_array($assignedDiscriminatorMaps)) {
                $assignedDiscriminatorMaps = array();
            }

            if (
                !in_array($classMetadata->name, $assignedDiscriminatorMaps)
                && !\XLite\Core\Database::getCacheDriver()->fetch('prefixAssigned_' . $classMetadata->name)
            ) {

                if ($classMetadata->discriminatorMap) {
                    $assignedDiscriminatorMaps = array_merge(
                        $assignedDiscriminatorMaps,
                        $classMetadata->discriminatorMap
                    );
                    \XLite\Core\Database::getCacheDriver()->save('assignedDiscriminatorMaps', $assignedDiscriminatorMaps);
                }

                $classMetadata->setTableName($prefix . $classMetadata->getTableName());
                $prefixAssigned[] = $classMetadata->name;
                \XLite\Core\Database::getCacheDriver()->save('prefixAssigned_' . $classMetadata->name, true);
    ...
    protected function postprocessUpdateSchema($schema)
    {
        if (preg_match('/^CREATE TABLE /Ss', $schema)) {
            ...
            $check = preg_match(
                '/^DROP TABLE ' . $this->getTablePrefix() . '(?:' . implode('|', $this->unmanagedTables) . ')$/Ss',
                $schema
            );
