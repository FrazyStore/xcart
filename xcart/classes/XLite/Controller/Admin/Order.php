...
class Order extends \XLite\Controller\Admin\AAdmin
{
     ...
     * @param boolean $forceCreate Force temporary order creation if it doesn't exist
     *
    ...
    public static function getTemporaryOrder($orderId, $force = true)
    {
        ...
        if (0 < $orderId) {
            $result = static::getTemporaryOrderFromCache($orderId) ?: ($force ? static::createTemporaryOrder($orderId) : null);
        }
    ...
    protected static function getTemporaryOrderFromCache($orderId)
    {
        ...
        if (isset(static::$tmpOrders[$orderId])) {
            ...
            if (!(static::$tmpOrders[$orderId]['order'] instanceOf \XLite\Model\Order)) {
                static::$tmpOrders[$orderId]['order']
                    = \XLite\Core\Database::getRepo('XLite\Model\Order')->find(intval(static::$tmpOrders[$orderId]['order']));
            }
    ...
    /**
     * Check - need send recalculat event or not
     * 
     * @param \XLite\Model\Order $order Order
     *  
     * @return boolean
    ...
    protected function doActionUpdate()
    {
        ...
        if ($this->getOrderChanges()) {
            ...
            \XLite\Core\OrderHistory::getInstance()->registerGlobalOrderChanges($order->getOrderId(), $this->getOrderChanges());
    ...
    protected function sendOrderChangeNotification()
    {
        if ($this->getSendNotificationFlag()) {
            \XLite\Core\Mailer::getInstance()->sendAdvancedChangedOrderCustomer($this->getOrder());
        }
    }
