...
class XPaymentsClient extends \XLite\Base\Singleton
{
    ...
    public function requestPaymentAdditionalInfo($txnId)
    {
        ...
        list($status, $response) = $this->getApiRequest('payment', 'get_additional_info', $data);
    ...
    public function requestPaymentCapture($txnId, \XLite\Model\Payment\Transaction $transaction, $amount = null)
    {
        ...
        return $this->getApiRequest('payment', 'capture', $request);
    }
    ...
    public function requestPaymentVoid($txnId, \XLite\Model\Payment\Transaction $transaction, $amount = null)
    {
        ...
        return $this->getApiRequest('payment', 'void', $request);
    }
    ...
    public function requestPaymentRefund($txnId, \XLite\Model\Payment\Transaction $transaction, $amount = null)
    {
        ...
        return $this->getApiRequest('payment', 'refund', $request);
    }
     ...
     * @param boolean                       forZeroAuth     Is this a zero-dollar authorization OPTIONAL
     *
     * @return array
     */
    public function prepareCart(\XLite\Model\Order $cart, \XLite\Model\Payment\Method $paymentMethod, $refId = null, $forceAuth = false, $forZeroAuth = false)
    {
        $config = \XLite\Core\Config::getInstance()->CDev->XPaymentsConnector;
        ...
        if (
            $forZeroAuth
            || \XLite\Core\Request::getInstance()->for_zero_auth
        ) {

            $result['items'][] = array(
                'sku'      => 'CardSetup',
                'name'     => $config->xpc_zero_auth_description ? $config->xpc_zero_auth_description: 'Card setup',
                'price'    => $this->roundCurrency($config->xpc_zero_auth_amount),
                'quantity' => '1',
            );

        } elseif ($cart->getItems()) {
    ...
    protected function sendAPIRequest($config, $xml)
    {
        ...
        if (
            ...
            $error = 'Communication error.' . PHP_EOL
                . 'Curl error #' . $errno . ': ' . $error . PHP_EOL;

            $this->getApiError($error);

            $result = array(false, nl2br($error));

        } elseif (substr($body, 0, 3) !== 'API') {

            // Check raw data

            $this->getApiError(
                'Response is not valid.' . PHP_EOL
                . 'Response headers: ' . var_export($headers, true) . PHP_EOL
                . 'Response: ' . var_export($body, true) . PHP_EOL
            );
