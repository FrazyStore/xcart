...
class NextPreviousProduct extends \XLite\View\AView
{
     ...
     */
    const DROPDOWN_ICON_WIDTH = '100';
    const DROPDOWN_ICON_HEIGHT = '100';
    ...
    protected function getNativeSessionKey()
    {
        ...
        if ($this->getItemsList() instanceof \XLite\Module\XC\ProductFilter\View\ItemsList\Product\Customer\Category\CategoryFilter) {
            $sessionKey = str_replace('\\', '', $itemsListClass)
                . \XLite\Module\XC\ProductFilter\View\ItemsList\Product\Customer\Category\CategoryFilter::PARAM_CATEGORY_ID;
        } else {
            $sessionKey = str_replace('\\', '', $itemsListClass);
        }
    ...
    protected function getNativeItemsList()
    {
        ...
        if (isset($cookieData['class'])) {
            $itemsListClass = $cookieData['class'];
            $listParams = \XLite\Core\Session::getInstance()->{str_replace('\\', '', $itemsListClass)}
                ? \XLite\Core\Session::getInstance()->{str_replace('\\', '', $itemsListClass)}
                : array();
