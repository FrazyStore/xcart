...
class ShipMethod extends \XLite\View\FormField\Select\Regular
{
    ...
    protected function getShippingOptions()
    {
        ...
        if (!$this->getModifier()->getMethod() && $this->getOrderEntity()->getShippingMethodName()) {
            $result[static::KEY_DELETED] = $this->getOrderEntity()->getShippingMethodName() . ' (' . static::t('deleted') . ')';
        }

        foreach ($this->getModifier()->getRates() as $rate) {
            $result[$this->getMethodId($rate)] = html_entity_decode(
