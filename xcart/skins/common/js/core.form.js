...
CommonForm.prototype.getDependency = function()
{
  ...
  this.$form.find('.has-dependency :input').each(
    function () {
      if ('undefined' != typeof(this.commonController)) {
        if ('undefined' != typeof(this.commonController.hasDependency) && this.commonController.hasDependency()) {
          dependency[this.commonController.element.name] = this.commonController.getDependency()
        }
      }
    }
  );
...
{
  var elementController = jQuery(this.form.elements.namedItem(field)).filter('[type!="hidden"]').get(0).commonController;
