...
core.bind(
  ...
  function (event, data)
  {
    ...
    core.get(
      $switch.attr('href'),
      function (data) {
        core.trigger('payment.methods.switch.loaded', {data: data, switch: $switch});
      }
    );
  }
);
...
core.bind(
  ...
  function (event, data)
  {
    if (data.data.responseJSON && data.data.responseJSON.href) {
      data.switch.attr('href', data.data.responseJSON.href);
      data.switch.closest('.switch').addClass(data.data.responseJSON.addCSS).removeClass(data.data.responseJSON.removeCSS);
    }
  }
);
