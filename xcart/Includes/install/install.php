 ...
 * @return void
 */
function fatal_error_extended($txt, $additionalNote = '')
{
    global $fatalErrorMsg;
        ...
        // Check if database settings provided are valid
            ...
            if ($connection) {
                ...
                if (!checkMysqlVersion($mysqlVersionErr, $currentMysqlVersion, true)) {
                    fatal_error_extended($mysqlVersionErr . (!empty($currentMysqlVersion) ? '<br />(current version is ' . $currentMysqlVersion . ')' : ''), xtr('kb_note_mysql_issue'));
                    $checkError = true;
                }
                ...
                fatal_error_extended(xtr('Can\'t connect to MySQL server specified:pdoerr<br /> Press \'BACK\' button and review MySQL server settings you provided.', array(':pdoerr' => (!empty($pdoErrorMsg) ? ': ' . $pdoErrorMsg : ''))), xtr('kb_note_mysql_issue'));
                $checkError = true;
            }
        }
